...
We also uploaded the "makeAllStepsToRegressionAutosomalPCA.py", that perform the same analysis than "makeAllStepsToRegression.py" but using the Autosomal PCA. This changed was made based on the reviewer suggestion. This script needs the autosomal dataset (should be send using the -g) and we recomend to use the "<name>_Relatedness" from the harmonization pipeline.
